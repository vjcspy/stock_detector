...
export const retrieveCashFlow = async (
  code: string,
  termType: number,
  page: number,
) => {
  ...
  try {
    const res = await fetch('https://finance.vietstock.vn/data/financeinfo', {
      ...
      body: `Code=${code}&ReportType=LC&ReportTermType=${termType}&Unit=1000000000&Page=${page}&PageSize=4&__RequestVerificationToken=${csrf}`,
      method: 'POST',
    });
